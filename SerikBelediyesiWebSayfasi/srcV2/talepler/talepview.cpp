      ...
      mPublicLink(false)
{
...

    {
        ...
        {
            container->addWidget (cpp14::make_unique<WText>("Bu Sayfa Sadece Bilgilendirme Amaçlıdır"));
            container->addWidget (cpp14::make_unique<WBreak>());
        }
        {
            container->addWidget (cpp14::make_unique<WText>("Resmi Olarak Kullanılamaz"));
            container->addWidget (cpp14::make_unique<WBreak>());
        }
...
void TalepView::initTalepView()
{
...

    {
        ...
        for( auto item : cursor )
        {
            ...
            if( item.userType () == TalepSubItem::PersonelUser ){
                ...
                titleContainerTC->setPositionScheme (PositionScheme::Absolute);
                titleContainerTC->setOffsets (WLength(0),Side::Right|Side::Top);
                titleContainerTC->setAttributeValue (Style::style,Style::background::color::color(Style::color::Red::Crimson));
                titleContainerTC->setWidth (20);
                titleContainerTC->setHeight (20);
                ...
                titleContainer->setAttributeValue (Style::style,Style::background::color::color(Style::color::Grey::DimGray)+
                                                   Style::color::color (Style::color::White::Azure));
                auto titleText = titleContainer->addWidget (cpp14::make_unique<WText>(item.tcName ().toStdString ()+
                                                                                      " / "+
                                                                                      item.tarih ().toStdString ()+
                                                                                      " " + item.saat ().toStdString ()+
                                                                                      " " + item.typeStr ().toStdString () ));
                ...
                titleContainer->setAttributeValue (Style::style,Style::background::color::color(Style::color::Grey::DimGray)+
                                                   Style::color::color (Style::color::White::Azure));
                auto titleText = titleContainer->addWidget (cpp14::make_unique<WText>(std::string(MAKE_STRINGS(TalepSubItem::NoUser))+" / "+
                                                                                      item.tarih ().toStdString ()+
                                                                                      " " + item.saat ().toStdString ()+
                                                                                      " " + item.typeStr ().toStdString () ));
                titleText->addStyleClass ("textShadow");
...
void TalepView::initTalepVatandasView()
{
    this->Content ()->clear ();
    this->Content ()->setMargin (20,Side::Top|Side::Bottom);
    this->Content ()->setWidth (WLength("100%"));
...
void TalepView::initTalepVatandasCevap()
{
    this->Footer ()->clear ();
    this->Footer ()->setMargin (20,Side::Top);
    this->Footer ()->setMargin (150,Side::Bottom);
...

    {
        ...
        auto fotoEkleMenu = popup->addItem("Fotoğraf Ekle");
        fotoEkleMenu->triggered().connect([=] {
            this->addEventVatandasItem (TalepSubItem::ItemType::Fotograf);
        });
        fotoEkleMenu->decorationStyle ().setCursor(Cursor::PointingHand);
        ...
        auto pdfEkleMenu = popup->addItem("Pdf Ekle");
        pdfEkleMenu->triggered().connect([=] {
            this->addEventVatandasItem (TalepSubItem::ItemType::Pdf);
        });
        pdfEkleMenu->decorationStyle ().setCursor (Cursor::PointingHand);
...

    {
        ...
        _container->clicked ().connect ([=](){
            ...
            this->setDurum (TalepKey::DurumKey::Tamamlandi.c_str ());
            if( this->updateTalep (this) )
            {
                TalepSubItem *item = new TalepSubItem;
                item->setType (TalepSubItem::ItemType::Log);
                item->setLog ("Talep/Şikayet Vatandaş Tarafından Tamamlandı");
                item->setTalepOid (this->oid ());
...

    {
        ...
        _container->decorationStyle ().setCursor (Cursor::PointingHand);
        _container->clicked ().connect ([=](){
            this->addEventVatandasItem (TalepSubItem::ItemType::Aciklama);
        });
    }
}
    ...
    case TalepSubItem::ItemType::Pdf:
    {
        ...
        ekleBtn->clicked().connect([=](){
            ...
            if( this->insertTalepSubItem (item) ){
                ...
                this->showMessage ("Uyarı","Talep Güncellenemedi");
            }
...
void TalepView::addEventVatandasItem(TalepSubItem::ItemType type_)
{
    if( this->durum ().toStdString () == SerikBLDCore::TalepKey::DurumKey::TeyitEdilmemis ||
            this->durum ().toStdString () == SerikBLDCore::TalepKey::DurumKey::Tamamlandi ||
            this->durum ().toStdString () == SerikBLDCore::TalepKey::DurumKey::RedEdildi )
