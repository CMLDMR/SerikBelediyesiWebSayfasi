...
void SikayetYonetimWidget::initSikayetler(const std::string &durumFilter)
{
    ...
    for( auto item : list )
    {
        ...
        if( _Vdurum ){ durum = std::to_string(count++)+ " " + _Vdurum.value().get_string().value.data();}
        if( _Vtarih ){ tarih = _Vtarih.value().get_string().value.data();}
        if( _Vmahalle ){ mahalle = _Vmahalle.value().get_string().value.data();}
        if( _Vbirim ){ birim = _Vbirim.value().get_string().value.data();}
        if( _Vkategori ){ kategori = _Vkategori.value().get_string().value.data();}
        if( _VadSoyad ){ adSoyad = _VadSoyad.value().get_string().value.data();}
...
void SikayetYonetimWidget::Sorgula()
{
    ...
    tSorgulaBtn->clicked().connect([=](){
        ...
        if( tComboBox->currentIndex() == 0 ){
            ...
            if( tcitem )
            {
                this->initSikayetlerBySahibi(tcitem.value()->Element(TC::KEY::tcno).value().get_string().value.data());
